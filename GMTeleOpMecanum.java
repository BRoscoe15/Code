...
*/
        ...
        if (gamepad2.right_trigger >= 0.10) {
            ...
            robot.scissorLift.setTargetPosition(robot.SCISSOR_DOWN_POS);
            pullupspeed = gamepad2.right_trigger * robot.SCISSOR_MOTOR_SPEED_FACTOR;
        ...
        if (gamepad2.left_trigger >= 0.10) {
            ...
            robot.scissorLift.setTargetPosition(robot.SCISSOR_UP_POS);
            pullupspeed = gamepad2.left_trigger * robot.SCISSOR_MOTOR_SPEED_FACTOR;
            robot.scissorLift.setPower(pullupspeed);
            scissorLiftMove = true;
        }

        if (gamepad2.left_trigger < 0.10 && gamepad2.right_trigger < 0.10 && scissorLiftMove) {

            robot.scissorLift.setTargetPosition(robot.scissorLift.getCurrentPosition());
            robot.scissorLift.setPower(robot.NULL_MOTOR_MOVE_SPEED);
            scissorLiftMove = false;
        }
        ...
        if (!gamepad2.left_bumper && !gamepad2.right_bumper && moveBoxMover) {
